...
+ [Управление памятью в языке Си](3rd-task.md#Управление_памятью-в-языке-Си)
+ [Общие рекомендации по управлению памятью](3rd-task.md#Общие-рекомендации-по-управлению-памятью)
+ [Многомерные массивы и динамическая память](3rd-task.md#Многомерные-массивы-и-динамическая-память)
+ [Структуры, объединения, typedef](3rd-task.md#Структуры-объединения-typedef)
+ [Файловый ввод\вывод](3rd-task.md#Файловый-вводвывод)
+ [Обработка ошибок, stderr и функция exit](3rd-task.md#Обработка-ошибок-stderr-и-функция-exit)
+ [Заголовочные файлы: взгляд внутрь](3rd-task.md#Заголовочные-файлы-взгляд-внутрь)
+ [Задания](3rd-task.md#Задания)
...
[К оглавлению](3rd-task.md#задача-3-структуры-управление-памятью-заголовочные-файлы)
...
Правила для этого просты: перед чтением/записью требуется **открыть** файл с помощью библиотечной функции ```fopen``` из ```<stdio.h>```, которая требует имя файла и возвращающая **файловый указатель** ```FILE*```, который в дальнейшем и будет использоваться для чтения/записи.
...
```bash
gcc main.c add.c subtract.c multiply.c divide.c -o math_program 
./math_program
```
...
6. Даны два текстовых файла. Реализовать программу ```cat```, которая конкатенирует содержимое файлов и выводит на экран или в другой файл.
