...
- [Параллельная технология MPI](#параллельная-технология-mpi)
  ...
  - [Понятие параллельной программы на MPI. Компиляция и запуск параллельной программы в среде MPI](#понятие-параллельной-программы-на-mpi-компиляция-и-запуск-параллельной-программы-в-среде-mpi)
...
- MPI может использоваться совместно с другими технологиями параллельного программирования, такими как OpenMP или [CUDA](https://developer.nvidia.com/blog/introduction-cuda-aware-mpi/), что позволяет создавать гибридные параллельные программы (в том числе, для [обучения нейронных сетей](https://docs.pytorch.org/tutorials/intermediate/dist_tuto.html)).
...
## Понятие параллельной программы на MPI. Компиляция и запуск параллельной программы в среде MPI

Под **параллельной программой** в рамках MPI понимается **множество одновременно выполняемых процессов**. Процессы могут выполняться на разных процессорах, но на одном процессоре могут располагаться сразу несколько процессов (либо процессор имеет **логические ядра**, либо процессы будут исполняться в режиме разделения времени). Таким образом, параллельная программа на MPI реализует модель **SPMP (Single Program Multiple Processes)**.
...
Код параллельной программы представляется в виде файла с расширением ```.c\.cpp``` и должен содержать в секции подключение заголовочных файлов ```#include <mpi.h>```. Для запуска программы (например, ***task.c***) требуется компилировать с помощью компилятора **mpicc** следующим образом:
...
Важным понятием технологии является **коммуникатор**, под которым понимают специально создаваемый служебный объект, объединяющий в своем составе группу процессов и ряд доп. параметров (контекст). Сразу после вызова функции *MPI_Init* формируется и становится доступным **глобальный коммуникатор** ```MPI_COMM_WORLD```. Такой коммуникатор содержит все доступные процессы. В ходе работы программы могут быть созданы и другие коммуникаторы, содержащие подмножество процессов, причем один и тот же процесс может принадлежать нескольким коммуникаторам одновременно.
