# Указатели, массивы, строки: память в языке Си

## Содержание

+ [Как устроена память. Указатели: прямые операции с памятью](pointers_arrays_memory.md#Как-устроена-память-Указатели-прямые-операции-с-памятью)
+ [Массивы в языке Си и адресная арифметика](pointers_arrays_memory.md#Массивы-в-языке-Си-и-адресная-арифметика)
+ [Многомерные массивы и массивы указателей](pointers_arrays_memory.md#Многомерные-массивы-и-массивы-указателей)
+ [Управление памятью в языке Си](pointers_arrays_memory.md#управление-памятью-в-языке-си)
+ [Общие рекомендации по управлению памятью](pointers_arrays_memory.md#Общие-рекомендации-по-управлению-памятью)
+ [Многомерные массивы и динамическая память](pointers_arrays_memory.md#Многомерные-массивы-и-динамическая-память)
+ [Регистровые переменные](pointers_arrays_memory.md#Регистровые-переменные)
+ [Строки в языке Си](pointers_arrays_memory.md#Строки-в-языке-Си)
+ [Аргументы командной строки: argc и argv](pointers_arrays_memory.md#Аргументы-командной-строки-argc-и-argv)
+ [Обработка ошибок, stderr и функция exit](pointers_arrays_memory.md#Обработка-ошибок-stderr-и-функция-exit)
+ [Файловый ввод\вывод](pointers_arrays_memory.md#Файловый-вводвывод)
...
var = 20;
// при изменении через указатель переменная будет хранить значение 30
*ptr = 30;
```
...
[К оглавлению](pointers_arrays_memory.md#указатели-массивы-строки)
...
[К оглавлению](memory_structs_etc.md#структуры-управление-памятью-заголовочные-файлы)
...
В ```<stdlib.h>``` есть некоторые полезные функции, которые, например, позволяют преобразовывать строки в типы *int* (```atoi```), *long* (```atol```), *double*, (```atof```), и схожие по действию функции `strtol`, `strtoul`, `strtod`.

[К оглавлению](pointers_arrays_memory.md#указатели-массивы-строки)

## Аргументы командной строки: argc и argv

В языке Си существует способ передавать аргументы или параметры командной строки при запуске программы на выполнение.
...
[К оглавлению](pointers_arrays_memory.md#структуры-управление-памятью-заголовочные-файлы)
